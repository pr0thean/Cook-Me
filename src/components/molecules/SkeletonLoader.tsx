...
const SkeletonLoader = ({ type }: Props) => {
    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="grid grid-cols-2 gap-4 p-4 md:grid-cols-4 md:gap-6 md:p-6">
          {[...Array(4)].map((_, index) => (
            <div key={index} className="aspect-square animate-pulse rounded-sm bg-gray" />
          ))}
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 px-4 md:grid-cols-2 md:gap-6 md:px-6">
          {[...Array(4)].map((_, index) => (
            <div key={index} className="h-48 animate-pulse rounded-sm bg-gray" />
          ))}
        </div>
      </div>
    )
  ...
  return (
    <div className="relative min-h-screen">
      <div className="h-96 animate-pulse bg-gray bg-linear-to-b from-transparent via-black/10 to-black" />
